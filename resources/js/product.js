...
(async () => {
    try {
        ...
        const megaMenuContainer = document.querySelector('.megamenu'),
            lastMegaMenuChild =
                megaMenuContainer.children[
                    megaMenuContainer.children.length - 1
                ],
        ...
        if (window.location.pathname === path.resolve('product', 'list')) {
            ...
            try {
                const response = await axios.get(
                    path.resolve('product', 'rated')
                );
                if (response.data) {
                    likedProducts = response.data;
                    showFavorite = true;
                }
            } catch (err) {
            ...
            img02.alt = '';
            discountBadgeContainer.classList.add(
                'product-badge',
                'offer-badge'
            );
            ...
            const byHighestRated = (a, b) => {
                return b.rating_total - a.rating_total;
            };
            ...
            const showProducts = (pageNumber, productsArr) => {
                ...
                productsPage.forEach(product => {
                    ...
                    productListContainer.appendChild(
                        singleItem.cloneNode(true)
                    );

                    if (showFavorite) {
                        const tempFavoriteButton = document.querySelector(
                            `[data-value="${productId}"].favme`
                        );

                        if (likedProducts.indexOf(productId) !== -1) {
                            tempFavoriteButton.classList.add('active');
                        }
                        tempFavoriteButton.addEventListener(
                            ...
                            async e => {
                                try {
                                    ...
                                    if (response.data) {
                                        const rateIndex = likedProducts.indexOf(
                                            response.data.product_id
                                        );
                                        if (response.data.rating_level === 1) {
                                            rateIndex === -1 &&
                                                likedProducts.push(
                                                    response.data.product_id
                                                );
            ...
            const filterProducts = () => {
                filtedProducts = sortedProducts.filter(product => {
                    ...
                    const categoryFilter =
                        currentCategoryId === 0
                            ? true
                            : product.categories.indexOf(currentCategoryId) !==
                              -1;
            ...
            categories.data.forEach(lv1Cate => {
                ...
                lv1Cate.categories.forEach(lv2Cate => {
                    ...
                    categoryContainerClone.children[1].appendChild(
                        categoryClone
                    );
                });
            ...
            const updatePrice = () => {
                ...
                if (!found) {
                    switch (lang) {
                        case 'vi':
                            price.innerText =
                                'Loại sản phẩm này không tồn tại !!!';
                            break;
                        default:
                            price.innerText =
